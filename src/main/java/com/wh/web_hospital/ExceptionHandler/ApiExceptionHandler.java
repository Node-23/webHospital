...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler{
    ...
    public ResponseEntity<Object> handleServices(ServicesExceptions ex, WebRequest request){
        var status = HttpStatus.BAD_REQUEST;
        var exception = new Exception();
        exception.setStatus(status.value());
        exception.setTitle(ex.getMessage());
        exception.setDateAndTime(LocalDateTime.now());
        return super.handleExceptionInternal(ex, exception, new HttpHeaders(), status, request);
    }
